...
use u_bbs::{groupgen, secret_keygen, server_to_user_zkp_iss, server_zkp_verify_iss, user_to_server_zkp_iss, user_zkp_verify_iss, user_to_server_zkp_pres, server_zkp_verify_pres};
...
fn zkp_verifies() {
    ...
    let proof = user_to_server_zkp_iss(test_group.clone(), test_message.clone(), s).unwrap();
    ...
    let rp = Scalar::random(&mut csprng);
    let a = s * ap;
    let ap = r * rp * a;

    let proof = user_to_server_zkp_pres(test_group.clone(), test_message.clone(), a, e, r, rp).unwrap();
    assert!(server_zkp_verify_pres(test_group.clone(), ap, test_sk, e, proof) == true)
}